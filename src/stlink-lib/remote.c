 ...
 * probe -- so the server applies the supplied ap before each operation.
 */
...
stlink_t *stlink_open_remote(int32_t verbose, const char *host, int32_t port,
    ...
    // Failure to reach the server is a fatal, user-facing error. Report it to
    // stderr directly rather than through ELOG, so it is shown even when the
    // tool runs at a log level that suppresses errors (e.g. st-info uses 0).
    ...
    // Mirror the tail of stlink_open_usb(): the connect sequence is
    // transport-agnostic and drives the backend ops over the wire.
    ...
    // If any RPC above hit a TCP error the server is gone; don't hand back a
    // dead handle. A legitimate target_connect failure leaves the link alive
    // and is handled downstream, as with stlink_open_usb().
    if (rl->dead) {
...
int32_t stlink_remote_serve(stlink_t *sl, int32_t fd) {
    ...
    for (;;) {
        ...
        case RPC_READ_MEM32:
            ...
            // rplen must match the bytes actually read into q_buf, not the raw
            // 32-bit arg (the backend length is uint16), or a bogus large a1
            // would make send_all() read past q_buf.
        ...
        case RPC_WRITE_MEM8:
            // The payload received must match the length the backend will read
            // from q_buf, or it would program stale buffer contents.
