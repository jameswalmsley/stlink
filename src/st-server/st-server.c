 ...
 * another machine can drive it via --remote. The server is a thin executor of
 * backend operations; all the high-level logic runs on the client.
 */
...
int32_t main(int32_t argc, char **argv) {
    ...
    // This server owns exactly one probe for its lifetime. Open it before
    // listening so a running server always means the requested ST-LINK was
    // claimed successfully. stlink_open_usb() also runs target chip-discovery,
    // which can print a harmless "Can not connect to target" error for parts
    // that need the client's connect mode (e.g. STM32H5 on AP1). The server
    // never uses the chip id -- the client does its own discovery -- so quiet
    // the open unless --verbose was requested.
